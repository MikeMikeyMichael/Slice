...
function gdpPerCapitaForYearUSD(y) {
  ...
  return (gdpBn * 1000000000) / (popM * 1000000);
}
...
function SliceIndexCalculator() {
  ...
  const [amount, setAmount] = useState("75000"); // FDR salary default
  const [year, setYear] = useState(1939);
  const [rows, setRows] = useState([]);
  const [latestGDPpc, setLatestGDPpc] = useState((() => {
      const pop2020 = POP_DECENNIAL_M[2020];
      const gdp2024 = GDP_LOOKUP_BN[2024] * 1000000000;
      const pop2024 = pop2020 * Math.exp(Math.log(1.01) * 4);
      return Math.round(gdp2024 / (pop2024 * 1000000));
    })()
  );
  ...
  const cpiForYear = useMemo(() => customCPI !== "" ? Number(customCPI) : CPI_U_ANNUAL[year], [year, customCPI]);
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        <div className="mb-4 inline-flex overflow-hidden hairline rounded-2xl">
        ...
        <section className="mt-8">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <ExampleCard title="FDR presidential salary (1939)" preset={{ amount: 75000, year: 1939, note: "Presidential salary was $75k from 1909 to 1948. Using 1939 year." }} />
            <ExampleCard title="Hoover Dam (midpoint 1936)" preset={{ amount: 165000000, year: 1936, note: "Often quoted near $165m. One year midpoint for illustration." }} />
            <ExampleCard title="Manhattan Project (peak 1944)" preset={{ amount: 930000000, year: 1944, note: "Peak year outlay to contrast SLICE person years vs CPI uplift." }} />
          </div>
        </section>
    ...
    <div className="panel p-5 mb-6">
      ...
      <div className="mt-4 grid md:grid-cols-3 gap-4">
        <div className="soft p-3">
          ...
          <div className="text-2xl font-bold">{singleSlicePersonYears == null ? "--" : singleSlicePersonYears.toLocaleString(undefined, { maximumFractionDigits: 0 })}</div>
...
/* Example cards */
function ExampleCard({ title, preset }) {
  return (
    <div className="soft rounded-2xl p-4">
