/* SLICE Index - US GDP per capita - black and white */
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" aria-hidden="true">
      <g transform="translate(32,32)">
        <circle cx="0" cy="0" r="28" fill="none" stroke="#000" strokeWidth="1.5"/>
...
/* US population decennial anchors (millions). Exponential interpolation between anchors. */
...
function popForYearMillionsExp(y: number): number | null {
  ...
  for (let i=0;i<years.length-1;i++) { if (y >= years[i] && y <= years[i+1]) { lo = years[i]; hi = years[i+1]; break; } }
  const p0 = POP_DECENNIAL_M[lo], p1 = POP_DECENNIAL_M[hi];
  const t = (y - lo) / (hi - lo);
  const growth = Math.log(p1/p0);
  return p0 * Math.exp(growth * t);
}
...
function SliceIndexCalculator() {
  ...
  const [amount, setAmount] = useState<string>("75000"); // FDR salary default
  const [year, setYear] = useState<number>(1939);
  const [rows, setRows] = useState<{ year: number; amount: string }[]>([]);
  const [latestGDPpc, setLatestGDPpc] = useState<number>(
    (() => {
      const pop2020 = POP_DECENNIAL_M[2020];
      const gdp2024 = GDP_LOOKUP_BN[2024] * 1_000_000_000;
      const pop2024 = pop2020 * Math.exp(Math.log(1.01) * 4);
      return Math.round(gdp2024 / (pop2024 * 1_000_000));
    })()
  );
  const [customGDPpcYear, setCustomGDPpcYear] = useState<string>("");
  const [customGDPpcValue, setCustomGDPpcValue] = useState<string>("");
  const [customCPI, setCustomCPI] = useState<string>("");

  useEffect(() => {
    function onLoadExample(ev: any) {
      if (!ev || !ev.detail) return;
      const { amount, year } = ev.detail;
      if (amount != null) setAmount(String(amount));
      if (year != null) setYear(Number(year));
      setTab("single");
      window.scrollTo({ top: 0, behavior: "smooth" });
    }
    window.addEventListener("loadExample", onLoadExample as any);
    return () => window.removeEventListener("loadExample", onLoadExample as any);
  }, []);

  const gdpPcForYear = useMemo(() => {
    if (customGDPpcYear !== "" && Number(customGDPpcYear) === year && customGDPpcValue !== "") {
      return Number(customGDPpcValue);
    }
    return gdpPerCapitaForYearUSD(year);
  }, [year, customGDPpcYear, customGDPpcValue]);
  ...
  const singleSlicePersonYears = useMemo(() => {
  ...
  const singleSliceToday = useMemo(() => {
    if (singleSlicePersonYears == null) return null;
    return singleSlicePersonYears * latestGDPpc;
  }, [singleSlicePersonYears, latestGDPpc]);
  ...
  const multiSlicePersonYears = useMemo(() => {
    let sum = 0;
    for (const r of rows) {
  ...
  const multiSliceToday = useMemo(() => multiSlicePersonYears * latestGDPpc, [multiSlicePersonYears, latestGDPpc]);
  ...
  if (tab === "single") {
    if (!gdpPcForYear && customGDPpcValue === "") warnings.push("No built in GDP per capita for this year. Provide an override.");
    if (!CPI_U_ANNUAL[year] && customCPI === "") warnings.push("No CPI for this year. Add CPI to enable CPI comparison.");
  }

  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-10 bg-white/90 backdrop-blur divider">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <SliceLogo size={28} />
            <h1 className="text-xl sm:text-2xl font-semibold tracking-tight">SLICE Index - US GDP per capita <span className="ml-2 align-middle badge">beta</span></h1>
          </div>
          <div className="flex items-center gap-3 text-sm">
            <label className="inline-flex items-center gap-2">
              <span className="hidden sm:inline subtle">Latest GDP per capita</span>
              <input type="number" className="w-36 field" value={latestGDPpc} onChange={(e) => setLatestGDPpc(Number(e.target.value))} />
            </label>
            <span className="subtle">USD only</span>
            <a
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        <div className="mb-4 inline-flex overflow-hidden soft rounded-2xl">
          <button onClick={() => setTab("single")} className={`px-4 py-2 text-sm ${tab === "single" ? "tab-on" : "tab-off"}`}>Single year</button>
          <button onClick={() => setTab("multi")} className={`px-4 py-2 text-sm ${tab === "multi" ? "tab-on" : "tab-off"}`}>Multi year</button>
        </div>
        ...
        {tab === "single" ? (
          <SingleInputs
            ...
            setAmount={setAmount}
            setYear={setYear}
            customGDPpcYear={customGDPpcYear}
            setCustomGDPpcYear={setCustomGDPpcYear}
            customGDPpcValue={customGDPpcValue}
            setCustomGDPpcValue={setCustomGDPpcValue}
            customCPI={customCPI}
            setCustomCPI={setCustomCPI}
            warnings={warnings}
            singleSlicePersonYears={singleSlicePersonYears}
        ...
        <ResultsGrid
          personYears={tab === "single" ? (singleSlicePersonYears ?? 0) : (multiSlicePersonYears ?? 0)}
          sliceToday={tab === "single" ? (singleSliceToday ?? 0) : (multiSliceToday ?? 0)}
        ...
        <section className="mt-8 panel p-5">
          ...
          <div className="space-y-3 text-sm muted">
            <p><span className="font-medium">US GDP per capita basis only</span>. For a single year amount A in year y with GDP per capita y_pc, person years = A / y_pc. For multi year projects, compute sum(amount_t / y_pc_t). To express a today equivalent, multiply the summed person years by the latest GDP per capita.</p>
            <p><span className="font-medium">Population</span>: annual values are estimated with exponential interpolation between official US decennial anchors, which assumes a constant growth rate between census years.</p>
            <p><span className="font-medium">CPI</span> is shown for household affordability comparison (1913 plus). It does not measure project scale.</p>
          </div>
          <div className="grid sm:grid-cols-2 gap-6 mt-5">
            <div>
              <h3 className="font-semibold mb-1">Limits</h3>
              <ul className="list-disc ml-5 space-y-1 text-sm muted">
                <li>USD only. CPI begins 1913. Provide a CPI override for earlier years if needed.</li>
                <li>This is a burden lens, not a replication estimator.</li>
                <li>Latest GDP per capita can be edited to match your preferred vintage.</li>
              </ul>
            </div>
            <div>
              <h3 className="font-semibold mb-1">Data sources and vintages</h3>
              <ul className="list-disc ml-5 space-y-1 text-sm muted">
                <li>Nominal GDP 1790 to 2020: historical reconstructions. 2021 to 2024: BEA current dollar (approx).</li>
                <li>Population anchors: US census decennial counts. Interpolation is exponential between anchors.</li>
                <li>CPI U annual averages: BLS.</li>
              </ul>
            </div>
          </div>
        </section>
        ...
        <section className="mt-8">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <ExampleCard title="FDR presidential salary (1939)" preset={{ amount: 75_000, year: 1939, note: "Presidential salary was $75k from 1909 to 1948. Using 1939 year." }} />
            <ExampleCard title="Hoover Dam (midpoint 1936)" preset={{ amount: 165_000_000, year: 1936, note: "Often quoted near $165m. One year midpoint for illustration." }} />
            <ExampleCard title="Manhattan Project (peak 1944)" preset={{ amount: 930_000_000, year: 1944, note: "Peak year outlay to contrast SLICE person years vs CPI uplift." }} />
          </div>
        </section>
...
function SingleInputs(props: any) {
  const { amount, year, setAmount, setYear, customGDPpcYear, setCustomGDPpcYear, customGDPpcValue, setCustomGDPpcValue, customCPI, setCustomCPI, warnings, singleSlicePersonYears, singleSliceToday, cpiToday } = props;
  return (
    <div className="panel p-5 mb-6">
      <h2 className="text-lg font-semibold mb-4">Enter an amount and a year</h2>
      <div className="grid sm:grid-cols-3 gap-4">
        <label className="block">
          <span className="block text-sm subtle mb-1">Amount (USD)</span>
          <input type="number" inputMode="decimal" className="w-full field" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="e.g., 75000" />
        </label>
        <label className="block">
          <span className="block text-sm subtle mb-1">Year</span>
          <input type="number" className="w-full field" value={year} onChange={(e) => setYear(Number(e.target.value))} placeholder="e.g., 1939" />
        </label>
        <div className="grid grid-cols-1 gap-3">
          <label className="block">
            <span className="block text-sm subtle mb-1">Override GDP per capita for this year (USD)</span>
            <div className="flex gap-2">
              <input type="number" className="w-24 field" value={customGDPpcYear} onChange={(e) => setCustomGDPpcYear(e.target.value)} placeholder="year" />
              <input type="number" className="w-full field" value={customGDPpcValue} onChange={(e) => setCustomGDPpcValue(e.target.value)} placeholder="value" />
            </div>
          </label>
          <label className="block">
            <span className="block text-sm subtle mb-1">Override CPI for year (annual avg)</span>
            <input type="number" step="0.001" className="w-full field" value={customCPI} onChange={(e) => setCustomCPI(e.target.value)} placeholder="optional" />
          </label>
        </div>
      </div>
      {warnings.length > 0 && (
        <div className="mt-4 text-sm" style={{ color:"#5b3a00", background:"#fff8e6", border:"1px solid #f1d6a8", borderRadius:"12px", padding:"10px 12px" }}>
          <ul className="list-disc ml-5">{warnings.map((w: string, i: number) => (<li key={i}>{w}</li>))}</ul>
        </div>
      )}
      <div className="mt-4 grid md:grid-cols-3 gap-4">
        <div className="soft rounded-2xl p-3">
          <div className="text-xs subtle">SLICE person years</div>
          <div className="text-2xl font-bold">{singleSlicePersonYears == null ? "--" : (singleSlicePersonYears as number).toLocaleString(undefined, { maximumFractionDigits: 0 })}</div>
          <div className="text-xs muted mt-1">Amount divided by GDP per capita for the selected year.</div>
        </div>
        <div className="soft rounded-2xl p-3">
          <div className="text-xs subtle">SLICE today equivalent</div>
          <div className="text-xl font-semibold">{singleSliceToday == null ? "--" : prettyMoney(singleSliceToday)}</div>
        </div>
        <div className="soft rounded-2xl p-3">
          <div className="text-xs subtle">CPI uplift</div>
    ...
    <div className="panel p-5 mb-6">
      ...
      <div className="overflow-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="divider">
                <td className="py-2 pr-3"><input type="number" className="w-24 field" value={r.year} onChange={(e) => updateRow(i, "year", e.target.value)} /></td>
                <td className="py-2 pr-3"><input type="number" className="w-40 field" value={r.amount} onChange={(e) => updateRow(i, "amount", e.target.value)} /></td>
                <td className="py-2 pr-3"><button className="btn" onClick={() => removeRow(i)}>Remove</button></td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mt-3 flex gap-2">
        ...
        <button className="btn" onClick={() => setRows([])}>Clear</button>
      </div>
      <p className="text-xs muted mt-2">Paste annual outlays. The app sums person years: sum(amount_t / GDPpc_t) and multiplies by latest GDP per capita for a today equivalent.</p>
    </div>
  );
}

/* Results */
function ResultsGrid({ personYears, sliceToday, cpiToday, sliceMultiple, cpiMultiple, diffMultiple, dollarDiff }:
  { personYears: number; sliceToday: number; cpiToday: number | null; sliceMultiple: number | null; cpiMultiple: number | null; diffMultiple: number | null; dollarDiff: number | null }) {
  return (
    <div className="grid lg:grid-cols-3 gap-4 items-start">
      <div className="soft rounded-2xl p-4">
        <h3 className="text-base font-semibold mb-2">SLICE person years</h3>
        <div className="text-3xl font-bold tracking-tight">{isFinite(personYears) ? personYears.toLocaleString(undefined, { maximumFractionDigits: 0 }) : "--"}</div>
        <p className="text-sm muted mt-1">Equivalent person years of average output, summed across years if multi year.</p>
        <div className="mt-4 pt-4 divider">
          <div className="text-sm muted">Today equivalent via SLICE (GDP per capita)</div>
          <div className="text-xl font-semibold">{isFinite(sliceToday) ? sliceToday.toLocaleString(undefined, { style: "currency", currency: "USD", maximumFractionDigits: 0 }) : "--"}</div>
          <div className="mt-2 text-xs subtle">SLICE multiple: {sliceMultiple == null ? "--" : sliceMultiple.toLocaleString(undefined, { maximumFractionDigits: 2 }) + "x"} per past dollar</div>
        </div>
      </div>
      <div className="soft rounded-2xl p-4">
        <h3 className="text-base font-semibold mb-2">CPI uplift</h3>
        {cpiToday == null ? <p className="subtle text-sm">CPI not available for the reference year or not provided.</p> : (
          <div>
            <div className="text-xl font-semibold">{cpiToday.toLocaleString(undefined, { style: "currency", currency: "USD", maximumFractionDigits: 0 })}</div>
            <p className="text-sm muted mt-1">Household affordability view using CPI U annual average.</p>
          </div>
        )}
        <div className="mt-4 text-xs subtle">
          ...
          {dollarDiff == null ? null : (
            <div className="mt-1">SLICE minus CPI: {diffMultiple == null ? "--" : diffMultiple.toLocaleString(undefined, { maximumFractionDigits: 2 }) + "x"} ({(sliceToday != null && cpiToday != null) ? (sliceToday - cpiToday).toLocaleString(undefined, { style: "currency", currency: "USD", maximumFractionDigits: 0 }) : ""})</div>
          )}
        </div>
      </div>
      <div className="soft rounded-2xl p-4">
        ...
        <ul className="text-sm muted space-y-2">
          <li><span className="font-medium">SLICE</span> answers how large it was relative to average output per person then.</li>
          <li><span className="font-medium">CPI</span> answers what a household would need today to buy the same basket.</li>
          <li>For forward looking costs, use engineering estimates.</li>
        </ul>
      </div>
    </div>
  );
...
/* Example cards */
function ExampleCard({ title, preset }: { title: string, preset: { amount: number, year: number, note?: string } }) {
  return (
    <div className="soft rounded-2xl p-4">
      <h3 className="font-semibold mb-1">{title}</h3>
      <p className="text-sm muted mb-3">{preset.note}</p>
      <div className="flex items-center justify-between text-sm">
        <div>
          <div>Amount: <span className="font-medium">{preset.amount.toLocaleString(undefined, { style: "currency", currency: "USD", maximumFractionDigits: 0 })}</span></div>
          <div>Year: <span className="font-medium">{preset.year}</span></div>
        </div>
        <button className="btn" onClick={() => {
          const ev = new CustomEvent("loadExample", { detail: preset });
