// SLICE Index Calculator (Zero-build static, USD only)
const { useMemo, useState } = React;

// Minimal, refined mark
function SliceLogo({ size = 28 }: { size?: number }) {
  const stroke = "#0f172a"; // slate-900
  const fill = "#111827";   // near-black for wedge
  return (
...
// CPI-U annual averages (1913..2024; 2024 provisional)
...
// Nominal GDP lookup (billions USD) 1790..2024
...
// Trend GDP using prior 10-year log-linear fit
...
function SliceIndexCalculator() {
  ...
  const gdpForYearBn = useMemo(() => {
    return customGDPbn !== "" ? Number(customGDPbn) : GDP_LOOKUP_BN[year];
  }, [year, customGDPbn]);

  const cpiForYear = useMemo(() => {
    return customCPI !== "" ? Number(customCPI) : CPI_U_ANNUAL[year as keyof typeof CPI_U_ANNUAL];
  }, [year, customCPI]);

  const latestCPI = CPI_U_ANNUAL[2024];

  // Single-year SLICE share
  ...
  }, [amount, year, gdpForYearBn, denom]);

  const singleSliceToday = useMemo(() => {
    if (singleSliceShare == null) return null;
    return singleSliceShare * (latestGDPbn * 1_000_000_000);
  }, [singleSliceShare, latestGDPbn]);
  ...
  const multiSliceToday = useMemo(() => multiSliceShare * (latestGDPbn * 1_000_000_000), [multiSliceShare, latestGDPbn]);
  ...
  if (tab === "single") {
    ...
    if (!CPI_U_ANNUAL[year] && customCPI === "") warnings.push("No CPI for this year. Add a CPI value to enable CPI comparison.");
  }
      ...
      <header className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b border-slate-100">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-sm">
            ...
            <a href="mailto:michael@sienko.xyz?subject=SLICE%20Index%20paper%20updates&body=Hi%20Michael%2C%0A%0APlease%20add%20me%20to%20updates%20for%20the%20SLICE%20Index%20paper.%0A%0AName%3A%20%5BYour%20name%5D%0AAffiliation%3A%20%5BYour%20org%5D%0AUse%20case%3A%20%5BBrief%5D%0A%0AThank%20you!"
               className="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-3 py-1 hover:bg-slate-50">Get paper updates</a>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        ...
        <section className="mt-8 bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6">
          ...
          <div className="space-y-3 text-sm text-slate-700">
            <p><span className="font-medium">SLICE INDEX</span> measures historical burden. For a single year amount A in year y with GDP Y_y, SLICE = A / Y_y. For multi year projects, compute Σ(A_t / Y_t) and then multiply the total share by latest GDP if you want a today-equivalent.</p>
            <p><span className="font-medium">Denominator toggle</span>: Actual uses recorded nominal GDP. Trend uses a simple log-linear fit on the prior ten years to remove temporary surges. When trend is not computable the app falls back to actual.</p>
          ...
          <div className="grid sm:grid-cols-2 gap-6 mt-5">
            <div>
              ...
              <ul className="list-disc ml-5 space-y-1 text-sm">
                ...
                <li>Land values and unique status goods are treated as context, not replication candidates.</li>
              </ul>
            </div>
            <div>
              <h3 className="font-semibold mb-1">Data sources and vintages</h3>
              <ul className="list-disc ml-5 space-y-1 text-sm">
                <li>GDP 1790–2020: MeasuringWorth nominal GDP (millions converted to billions).</li>
                <li>GDP 2021–2023: BEA current-dollar GDP, calendar year. 2024 editable.</li>
                <li>CPI-U 1913–2024: BLS annual averages. 2024 provisional.</li>
              </ul>
            </div>
          </div>
          <div className="mt-5 grid sm:grid-cols-3 gap-6 text-sm">
            ...
            <div>
              ...
              <p>SLICE Index calculator, version 0.2 beta. Methodology per the working paper "The SLICE INDEX: Measuring Historical Project Scale by Economic Burden".</p>
            </div>
            <div>
              <h3 className="font-semibold mb-1">Feedback</h3>
              <p>Please double check numbers and <a href="mailto:team@sliceindex.org" className="underline">email us</a>. This is a beta.</p>
            </div>
          </div>
        </section>
        ...
        <section className="mt-8">
          ...
          <div className="grid md:grid-cols-3 gap-4">
            <ExampleCard title="Hoover Dam (full complex, 1936 midpoint)" preset={{ amount: 165_000_000, year: 1936, note: "Total complex often quoted 165m. Use midpoint year for a one-year illustration." }} onApply={(p) => { setTab("single"); setAmount(String(p.amount)); setYear(p.year); setCustomGDPbn(""); setCustomCPI(""); }} />
            <ExampleCard title="Presidential salary (Washington, 1790)" preset={{ amount: 25_000, year: 1790, note: "CPI not available. SLICE shows burden share only." }} onApply={(p) => { setTab("single"); setAmount(String(p.amount)); setYear(p.year); setCustomGDPbn(""); setCustomCPI(""); }} />
        ...
        <footer className="mt-10 text-xs text-slate-500 border-t border-slate-100 pt-4">
          <p><span className="font-semibold">Beta disclaimer</span>: This calculator is in beta. Always verify figures against your preferred data sources and read the methodology. Numbers may change when series are revised.</p>
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6 mb-6">
      ...
      <div className="mt-4 grid md:grid-cols-3 gap-4">
        <div className="bg-slate-50 rounded-xl p-3 border border-slate-200">
          ...
          <div className="text-2xl font-bold">{singleSliceShare == null ? "--" : pct(singleSliceShare)}</div>
        </div>
        <div className="bg-slate-50 rounded-xl p-3 border border-slate-200">
...
function MultiInputs({ rows, setRows }: { rows: { year: number; amount: string }[]; setRows: any }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6 mb-6">
      ...
      <p className="text-xs text-slate-600 mt-2">Tip: Paste annual outlays, then read the SLICE box below. The app sums Σ(A_t / Y_t) using the selected denominator.</p>
    </div>
  );
}

// Results and multipliers
function ResultsGrid({ sliceShare, sliceToday, cpiToday, sliceMultiple, cpiMultiple, diffMultiple, dollarDiff }:
  { sliceShare: number; sliceToday: number; cpiToday: number | null; sliceMultiple: number | null; cpiMultiple: number | null; diffMultiple: number | null; dollarDiff: number | null }) {
  return (
    <div className="grid lg:grid-cols-3 gap-4 items-start">
      <div className="bg-white/80 rounded-2xl border-0 shadow-none p-4 sm:p-6">
        ...
        <div className="text-3xl font-bold tracking-tight">{isFinite(sliceShare) ? pct(sliceShare) : "--"}</div>
        <p className="text-sm text-slate-600 mt-1">Share of one year of output, summed across selected years if multi year.</p>
        <div className="mt-4 pt-4 border-t border-slate-200">
          <div className="text-sm text-slate-600">Today equivalent via SLICE</div>
      ...
      <div className="bg-white/80 rounded-2xl border-0 shadow-none p-4 sm:p-6">
        ...
        {cpiToday == null ? <p className="text-slate-500 text-sm">CPI not available for the reference year or not provided.</p> : (
          <div>
            ...
            <p className="text-sm text-slate-600 mt-1">Household affordability lens using CPI-U annual average.</p>
          </div>
        )}
      ...
      <div className="bg-white/80 rounded-2xl border-0 shadow-none p-4 sm:p-6">
        ...
        <ul className="text-sm text-slate-700 space-y-2">
          <li><span className="font-medium">SLICE</span> answers: How large was this relative to the economy then.</li>
          <li><span className="font-medium">CPI</span> answers: What would a household need today to buy the same basket.</li>
          <li>Trend denominator uses a ten year log-linear fit prior to each year.</li>
        </ul>
        <div className="mt-4 text-xs text-slate-500">Tip: Keep latest GDP current. This controls the today-equivalent for SLICE.</div>
      </div>
    </div>
  );
}
...
// expose globally for the mount script
// @ts-ignore
