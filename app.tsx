/* SLICE Index - US GDP per capita only */
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 64 64" aria-hidden="true">
      <g transform="translate(32,32)">
        ...
        <path d="M0,0 L0,-26 A26,26 0 0 1 11.0,-23.0 Z" fill={fill} />
...
function SliceIndexCalculator() {
  ...
  // Defaults to FDR salary example
      ...
      <header className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b border-slate-100">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <SliceLogo size={28} />
            <h1 className="text-xl sm:text-2xl font-semibold tracking-tight">SLICE Index - US GDP per capita <span className="ml-2 align-middle text-[10px] font-medium px-1.5 py-0.5 rounded bg-slate-100 text-slate-700">beta</span></h1>
          </div>
          <div className="flex items-center gap-3 text-sm">
            <label className="inline-flex items-center gap-2">
              <span className="hidden sm:inline">Latest GDP per capita (USD)</span>
              <input type="number" className="w-36 rounded-xl border border-slate-300 px-3 py-1 focus:outline-none focus:ring-2 focus:ring-slate-300" value={latestGDPpc} onChange={(e) => setLatestGDPpc(Number(e.target.value))} />
            </label>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        <section className="mt-8 bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6">
          ...
          <div className="space-y-3 text-sm text-slate-700">
            ...
            <p><span className="font-medium">Population</span>: annual values are estimated with exponential interpolation between official US decennial anchors (constant growth rate between census years).</p>
            <p><span className="font-medium">CPI</span> is shown for household affordability comparison (1913+). It does not measure project scale.</p>
          </div>
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6 mb-6">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <label className="block">
          ...
          <input type="number" inputMode="decimal" className="w-full rounded-xl border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-300" value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="e.g., 75000" />
        </label>
        <label className="block">
          <span className="block text-sm text-slate-600 mb-1">Year</span>
          <input type="number" className="w-full rounded-xl border border-slate-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-slate-300" value={year} onChange={(e) => setYear(Number(e.target.value))} placeholder="e.g., 1939" />
        </label>
        <div className="grid grid-cols-1 gap-3">
          <label className="block">
            <span className="block text-sm text-slate-600 mb-1">Override GDP per capita for this year (USD)</span>
            <div className="flex gap-2">
              <input type="number" className="w-24 rounded-xl border border-slate-300 px-3 py-2" value={customGDPpcYear} onChange={(e) => setCustomGDPpcYear(e.target.value)} placeholder="year" />
              <input type="number" className="w-full rounded-xl border border-slate-300 px-3 py-2" value={customGDPpcValue} onChange={(e) => setCustomGDPpcValue(e.target.value)} placeholder="value" />
            </div>
          </label>
          <label className="block">
      ...
      <div className="mt-4 grid md:grid-cols-3 gap-4">
        <div className="bg-slate-50 rounded-xl p-3 border border-slate-200">
          <div className="text-xs text-slate-600">SLICE person-years</div>
          ...
          <div className="text-xs text-slate-500 mt-1">Amount divided by GDP per capita for the selected year.</div>
        </div>
        <div className="bg-slate-50 rounded-xl p-3 border border-slate-200">
...
function MultiInputs({ rows, setRows }: { rows: { year: number; amount: string }[]; setRows: any }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-4 sm:p-6 mb-6">
      ...
      <p className="text-xs text-slate-600 mt-2">Paste annual outlays. The app sums person-years: sum(amount_t / GDPpc_t) and multiplies by latest GDP per capita for a today equivalent.</p>
    </div>
  );
}
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-4 items-start">
      <div className="bg-white/80 rounded-2xl border-0 shadow-none p-4 sm:p-6">
        ...
        <p className="text-sm text-slate-600 mt-1">Equivalent person-years of average output, summed across years if multi year.</p>
        <div className="mt-4 pt-4 border-t border-slate-200">
          <div className="text-sm text-slate-600">Today equivalent via SLICE (GDP per capita)</div>
      ...
      <div className="bg-white/80 rounded-2xl border-0 shadow-none p-4 sm:p-6">
        ...
        <div className="mt-4 text-xs text-slate-500">
          ...
          {dollarDiff == null ? null : (
            <div className="mt-1">SLICE minus CPI: {diffMultiple == null ? "--" : diffMultiple.toLocaleString(undefined, { maximumFractionDigits: 2 }) + "x"} ({sliceToday != null and cpiToday != null ? (sliceToday - cpiToday).toLocaleString(undefined, { style: "currency", currency: "USD", maximumFractionDigits: 0 }) : ""})</div>
          )}
        </div>
      </div>
      <div className="bg-white/80 rounded-2xl border-0 shadow-none p-4 sm:p-6">
  ...
  return (
    <div className="bg-white/80 rounded-2xl border-0 shadow-none p-4">
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <button className="rounded-xl border border-slate-300 px-3 py-1 hover:bg-slate-50" onClick={() => {
